...
class Complex:
    ...
    def set_re(self, real):
        if isinstance(real, numbers.Number):
            self._re = real
        else:
            raise ValueError(f'{real} is not a number')

    def set_im(self, im):
        if isinstance(im, numbers.Number):
            self._im = im
        else:
            raise ValueError(f'{im} is not a number')
